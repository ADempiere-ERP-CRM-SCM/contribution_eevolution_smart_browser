...
public class WBrowser extends Browser implements IFormController, EventListener,  WTableModelListener, ValueChangeListener, ASyncProcess{
	...
	public WBrowser(boolean modal, int WindowNo, String value,
	...
	public void addComponent(MBrowseField field, Row row, String name, String title)
	{
		...
		{
			WStringEditor data = new WStringEditor(name, false, false, true, 30, 30, "", null);
		...
		{
			WNumberEditor data= new WNumberEditor(name, false, false, true, field.getAD_Reference_ID(), title);
			data.getComponent();
			row.appendChild(data.getComponent());
		}
	...
	private WSearchEditor getLookup(MBrowseField field) 
	{
		try 
		{
			...
			WSearchEditor data = new WSearchEditor(field.getAD_View_Column().getAD_Column().getColumnName(), field.isMandatory(), false, true, dataL);
	...
	private void setParameters()
	{
		...
		for(int index = 0; index<= rowList.size()-1; index++)
		{
			...
			{	
				...
				{	
					WSearchEditor component = (WSearchEditor) components.get(ind);
					addParameter(component.getColumnName(), component.getValue());
					continue;
				}	
				if(components.get(ind) instanceof WStringEditor)
				{	
					WStringEditor component  = (WStringEditor) components.get(ind);
					addParameter(component.getComponent().getName(), component.getValue());
					continue;
				}
				if(components.get(ind) instanceof Checkbox)
				{
					Checkbox component  = (Checkbox) components.get(ind);
					addParameter(component.getName(), new Boolean(component.isChecked()));
					continue;
				}
	...
	private boolean testCount()
	{
		...
		if (role.isQueryMax(no))
			return true; //use dialog //ADialog.ask(p_WindowNo, m_frame, "InfoHighRecordCount", String.valueOf(no));
		return true;
	}
